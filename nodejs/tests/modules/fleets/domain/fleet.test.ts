  ...
  describe('Invariants', () => {
    ...
    describe('name validation', () => {
      ...
      it('should fail if name is less than 2 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: 0, maxLength: LIMITS.NAME.MIN_LENGTH - 1 })
      ...
      it('should fail if maintenance budget is less than 0', () => {
        fc.assert(
          fc.property(fc.integer({ max: LIMITS.MAINTENANCE_BUDGET.MIN - 1 }), lowBudget => {
            ...
            expect(() => builder.create()).toThrow(
              `Maintenance budget must be at least ${LIMITS.MAINTENANCE_BUDGET.MIN}`
            )
          })
        )
      })
      ...
      it('should fail if maintenance budget is greater than 10000', () => {
        fc.assert(
          fc.property(fc.integer({ min: LIMITS.MAINTENANCE_BUDGET.MAX + 1 }), highBudget => {
            ...
            expect(() => builder.create()).toThrow(
              `Maintenance budget must be less than ${LIMITS.MAINTENANCE_BUDGET.MAX}`
            )
          })
        )
      })
    ...
    describe('type validation', () => {
      it('should throw if fleet type is invalid', () => {
        fc.assert(
          fc.property(
            fc.string().filter(s => !(s in FleetType)),
            invalidType => {
  ...
  describe('Business rules', () => {
    ...
    describe('on addition', () => {
      it('should not allow adding the same aircraft twice', () => {
        ...
        expect(() => fleet.addAircraft(aircraftId)).toThrow(
          `Aircraft with id ${aircraftId} is already part of the fleet`
        )
      })
    })
    ...
    describe('on retirement', () => {
      it('should not allow retiring an aircraft that is not part of the fleet', () => {
        ...
        expect(() => fleet.retireAircraft(aircraftId)).toThrow(
          `Aircraft with id ${aircraftId} is not part of the fleet`
        )
      })
