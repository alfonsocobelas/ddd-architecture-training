    ...
    describe('code validations', () => {
      ...
      it('should fail if normalized code is less than 2 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: 1, maxLength: LIMITS.CODE.MIN_LENGTH - 1 })
      ...
      it('should fail if normalized code exceeds 10 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: LIMITS.CODE.MAX_LENGTH + 1 })
            ...
            longCode => {
              ...
              expect(() => builder.create()).toThrow(
                `Code must be less than or equal to ${LIMITS.CODE.MAX_LENGTH} characters`
              )
            }
          )
        )
    ...
    describe('name validations', () => {
      ...
      it('should fail if name is less than 2 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: 1, maxLength: LIMITS.NAME.MIN_LENGTH - 1 })
              .filter(str => normalizeString(str).length > 0),
            shortName => {
    ...
    describe('manufacturer validations', () => {
      ...
      it('should fail if manufacturer is less than 2 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: 1, maxLength: LIMITS.MANUFACTURER.MIN_LENGTH - 1 })
              .filter(str => normalizeString(str).length > 0),
            shortManufacturer => {
              const builder = AircraftModelBuilder.aModel().withManufacturer(shortManufacturer)

              expect(() => builder.create()).toThrow(
                `Manufacturer must be at least ${LIMITS.MANUFACTURER.MIN_LENGTH} characters`
              )
            }
          )
        )
      ...
      it('should fail if manufacturer exceeds 20 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: LIMITS.MANUFACTURER.MAX_LENGTH + 1 })
    ...
    describe('engine validations', () => {
      it('should fail if number of engines is not an integer', () => {
        fc.assert(
          fc.property(
            fc
              .double({ min: LIMITS.ENGINES.MIN, max: LIMITS.ENGINES.MAX, noNaN: true })
        ...
        fc.assert(
          fc.property(
            ...
            invalidNumEngines => {
              ...
              expect(() => builder.create()).toThrow(
                `Number of engines must be greater than or equal to ${LIMITS.ENGINES.MIN}`
              )
            }
          )
        )
      ...
      it('should fail if number of engines is greater than 6', () => {
        fc.assert(
          fc.property(fc.integer({ min: LIMITS.ENGINES.MAX + 1 }), invalidNumEngines => {
            const builder = AircraftModelBuilder.aModel().withNumEngines(invalidNumEngines)

            expect(() => builder.create()).toThrow(
              `Number of engines must be less than or equal to ${LIMITS.ENGINES.MAX}`
            )
          })
        )
      })
    ...
    describe('passenger capacity validations', () => {
      it('should fail if passenger capacity is not an integer', () => {
        fc.assert(
          fc.property(
            fc
              .double({ min: LIMITS.PASSENGERS.MIN, max: LIMITS.PASSENGERS.MAX, noNaN: true })
              .filter(num => !Number.isInteger(num)),
            decimalCapacity => {
      ...
      it('should fail if passenger capacity is less than 1', () => {
        fc.assert(
          fc.property(fc.integer({ max: LIMITS.PASSENGERS.MIN - 1 }), invalidQty => {
            const builder = AircraftModelBuilder.aModel().withPassengers(invalidQty)

            expect(() => builder.create()).toThrow(
              `Passenger capacity must be greater than or equal to ${LIMITS.PASSENGERS.MIN}`
            )
          })
        )
      })
      ...
      it('should fail if passenger capacity is greater than 400', () => {
        fc.assert(
          fc.property(fc.integer({ min: LIMITS.PASSENGERS.MAX + 1 }), invalidQty => {
            const builder = AircraftModelBuilder.aModel().withPassengers(invalidQty)

            expect(() => builder.create()).toThrow(
              `Passenger capacity must be less than or equal to ${LIMITS.PASSENGERS.MAX}`
            )
          })
        )
      })
  ...
  describe('Business rules', () => {
    ...
    describe('on removal', () => {
      it('should not allow removing a model if it has associated aircraft', () => {
        fc.assert(
          fc.property(fc.integer({ min: 1, max: 10000 }), totalAircraftCount => {
            const model = AircraftModelMother.random()

            expect(() => model.ensureCanBeRemoved(totalAircraftCount)).toThrow(
              'Cannot remove model with associated aircraft'
            )
          })
        )
      })
