  ...
  describe('Invariants', () => {
    ...
    describe('tail number validation', () => {
      ...
      it('should fail is normalized tail number is less than 5 characters', () => {
        fc.assert(
          fc.property(
            fc.string({ minLength: 1, maxLength: LIMITS.TAIL_NUMBER.MIN_LENGTH - 1 })
              .filter(str => normalizeString(str).length > 0),
            (shortTailNumber) => {
              const builder = AircraftBuilder.anAircraft().withTailNumber(shortTailNumber)

              expect(() => builder.create()).toThrow(`Tail number must be at least ${LIMITS.TAIL_NUMBER.MIN_LENGTH} characters`)
            }
          )
        )
      })
      ...
      it('should fail if normalized tail number exceeds 20 characters', () => {
        fc.assert(
          fc.property(
            fc.string({ minLength: LIMITS.TAIL_NUMBER.MAX_LENGTH + 1 })
            ...
            (longTailNumber) => {
              ...
              expect(() => builder.create()).toThrow(`Tail number must be less than or equal to ${LIMITS.TAIL_NUMBER.MAX_LENGTH} characters`)
            }
          )
        )
      })
