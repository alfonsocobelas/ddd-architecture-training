...
import { SearchIssuesInputMother } from '../mothers/search-issues-input.mother'
import { SearchIssuesOutputMother } from '../mothers/search-issues-output.mother'
...
describe('SearchIssuesUseCase (unit tests)', () => {
  ...
  it('should return cursor in first call', async () => {
    ...
    const issues = repeat(() => IssueMother.avionics(), input.pageSize + 1)
  ...
  it('should return no cursor in last call', async () => {
    // GIVEN
    const input = SearchIssuesInputMother.firstCall()
    const issues = repeat(() => IssueMother.engine(), input.pageSize + 1)
    repository.givenMatching(issues) // limit + 1
  ...
  it('should throw error when invalid filter is provided', async () => {
    ...
    await expect(useCase.invoke(input))
      .rejects.toThrow('The field "invalidField" is not allowed for searching.')
  ...
  it('should throw error when invalid order is provided', async () => {
    ...
    // WHEN / THEN
    await expect(useCase.invoke(input))
      .rejects.toThrow('The field "invalidField" is not allowed for ordering.')
  ...
  it('should apply mutliple filters and orders correctly', async () => {
    ...
    const issues = repeat(() => IssueMother.avionics(), 11)
    repository.givenMatching(issues) // limit + 1
