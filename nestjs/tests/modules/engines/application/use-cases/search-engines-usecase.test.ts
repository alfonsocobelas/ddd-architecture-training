...
import { SearchEnginesUseCase } from 'src/contexts/operations/modules/engines/application/use-cases/search-engines-usecase.service'
import { SearchEnginesInputMother } from '../mothers/search-engines-input.mother'
import { SearchEnginesOutputMother } from '../mothers/search-engines-output.mother'
...
describe('SearchEnginesUseCase (unit tests)', () => {
  ...
  it('should throw error when invalid filter is provided', async () => {
    ...
    // WHEN / THEN
    await expect(useCase.invoke(input)).rejects.toThrow('The field "invalidField" is not allowed for searching.')
  ...
  it('should throw error when invalid order is provided', async () => {
    ...
    // WHEN / THEN
    await expect(useCase.invoke(input)).rejects.toThrow('The field "invalidField" is not allowed for ordering.')
    repository.assertNotCalled('matching')
  })
